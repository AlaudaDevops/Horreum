...
AUTH="-u horreum.bootstrap:secret"
...
post() {
  ...
  echo "POST $1" >&2
  OUTPUT=$(mktemp)
  curl -s $AUTH ${API}$1 -H 'Content-Type: '$TYPE -d "$BODY" > $OUTPUT
  if [ $? != 0 ]; then
     cat $OUTPUT >&2
     echo "" >&2
     fail
...
get() {
  curl -s $AUTH ${API}$1 -H 'Accept: application/json' || fail
}

delete() {
  echo "DELETE $1"
  curl -s $AUTH ${API}$1 -X DELETE || fail
}
